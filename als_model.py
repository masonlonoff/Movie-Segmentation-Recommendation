...
def start_spark():
    # What: Start a Spark session
    # Why: Required to use Spark APIs
    # How: Use SparkSession.builder
    return SparkSession.builder.appName("ALSModelEval").getOrCreate()


def load_data(spark, train_path, val_path, test_path):
    # What: Load pre-split train/val/test data
    # Why: Needed for training and evaluation
    # How: Use Spark's read.parquet
...
def train_als_model(train_df, val_df, ranks, reg_params):
    # What: Tune ALS model over hyperparameter grid
    # Why: Identify best performing ALS model
    # How: Use loop over ranks and regParams, evaluate on val set using NDCG
    ...
    for rank in ranks:
        for reg in reg_params:
            ...
            # Prepare predictions for RankingMetrics
...
def evaluate_model(model, test_df):
    # What: Evaluate final ALS model
    # Why: Report Precision, Recall, NDCG, MAP
    # How: Use Spark's RankingMetrics

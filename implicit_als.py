...
def train_als_model(train_df, val_df, ranks, reg_params, alphas):
    best_model = None
    best_ndcg = -1
    best_rank = None
    best_reg = None
    ...
    for rank in ranks:
        for alpha in alphas:
            for reg in reg_params:
                print(f"Training ALS (implicit) with rank={rank}, alpha = {alpha}, regParam={reg}", flush=True)
                als = ALS(
                    userCol="userId",
                    itemCol="movieId",
                    ratingCol="rating",
                ...
                print(f"Rank={rank}, RegParam={reg}, alpha = {alpha}, NDCG@100={ndcg:.4f}", flush=True)
    ...
    print(f"Best Implicit ALS - Rank: {best_rank}, RegParam: {best_reg}, alpha: {alpha}, NDCG@100: {best_ndcg:.4f}", flush=True)
    return best_model
...
def main():
    ...
    ranks = [25,26]
    reg_params = [0.125, 0.15]
    alphas = [1, 5, 10]
    best_model = train_als_model(train_df, val_df, ranks, reg_params, alphas)
